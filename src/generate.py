...
def gen(dataset_config, DIR):
    ...
    OUT_DOMAIN_SAMPLE_SPACE = dataset_config.gettuple(
        "OUT_DOMAIN_SAMPLE_SPACE")
    ...
    train = generate_dataset((TRAIN_SIZE, X_DIM), FUNC,
                             MU, SIGMA, sample_space=IN_DOMAIN_SAMPLE_SPACE)
    val = generate_dataset((VAL_SIZE, X_DIM), FUNC, MU,
                           SIGMA, sample_space=IN_DOMAIN_SAMPLE_SPACE)

    test_in_domain = generate_dataset(
        (TEST_SIZE, X_DIM), FUNC, MU, SIGMA, sample_space=IN_DOMAIN_SAMPLE_SPACE)
    test_out_domain = generate_dataset((TEST_SIZE, X_DIM), FUNC, MU, SIGMA,
                                       sample_space=OUT_DOMAIN_SAMPLE_SPACE, void_space=IN_DOMAIN_SAMPLE_SPACE)

    train_complete = np.hstack((train[0], train[1].reshape(-1, 1)))
    val_complete = np.hstack((val[0], val[1].reshape(-1, 1)))

    test_in_domain_complete = np.hstack(
        (test_in_domain[0], test_in_domain[1].reshape(-1, 1)))
    test_out_domain_complete = np.hstack(
        (test_out_domain[0], test_out_domain[1].reshape(-1, 1)))
    ...
    # Save datasets
    np.savetxt(f"{DIR}/{NAME}/train.csv", train_complete,
               delimiter=",")  # detach()? numpy()?
    np.savetxt(f"{DIR}/{NAME}/val.csv", val_complete, delimiter=",")
    np.savetxt(f"{DIR}/{NAME}/test_in_domain.csv",
               test_in_domain_complete, delimiter=",")
    np.savetxt(f"{DIR}/{NAME}/test_out_domain.csv",
               test_out_domain_complete, delimiter=",")
    ...
    train_sample_posterior = train_sample_dist.sample(10000).squeeze()

    fig, ax = plt.subplots(figsize=(15, 10))
    if train_sample_posterior.std() != 0:
        plot_distribution(train_sample_posterior, ax=ax)
    ax.axvline(sample_train_y, color="red",
